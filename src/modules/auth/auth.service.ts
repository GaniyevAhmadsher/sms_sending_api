...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    const user = await this.prisma.user.create({
      data: { email: dto.email, name: dto.name, passwordHash: this.hashPassword(dto.password) },
    });
  ...
  async refresh(refreshToken: string) {
    ...
    if (!stored || stored.revokedAt || stored.expiresAt.getTime() <= Date.now() || stored.jti !== payload.jti) {
  ...
  async googleLogin(idToken: string) {
    ...
    const payload = (await response.json()) as { sub?: string; email?: string; aud?: string; name?: string };
